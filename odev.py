    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            if (top_left[0] <= hand_center_x <= bottom_right[0] and 
                ...
                # Y koordinatı: Eğer elin merkezi kutunun üstündeyse, aşağı hareket etmeli
                if hand_center_y < top_left[1]:
                    dy = top_left[1] - hand_center_y  # Aşağı gitmesi gereken piksel
                # Y koordinatı: Eğer elin merkezi kutunun altındaysa, yukarı hareket etmeli
                elif hand_center_y > bottom_right[1]:
                    dy = bottom_right[1] - hand_center_y  # Yukarı gitmesi gereken piksel (negatif değer)
                
                # Yön ve mesafe bilgisini terminale yazdır
                hareket_x = ""
                hareket_y = ""
                ...
                """
                BU KISIMDA KAMERA İLE MOTORARASINDAKİ BAĞLANTI KODU EKLENCEK
                """
        #####################################################################################################
                if dx > 0:
                    hareket_x = f"{abs(dx)} piksel SAĞA git"
                elif dx < 0:
                    hareket_x = f"{abs(dx)} piksel SOLA git"
                
                if dy > 0:
                    hareket_y = f"{abs(dy)} piksel AŞAĞI git"
                elif dy < 0:
                    hareket_y = f"{abs(dy)} piksel YUKARI git"
        #####################################################################################################
                ...
                #### BURAYI SİLEBİLİRSİNİZ ###
                if hareket_x and hareket_y:
                    print(f"El DIŞARDA: {hareket_y}, {hareket_x}")
                elif hareket_x:
                    print(f"El DIŞARDA: {hareket_x}")
                elif hareket_y:
                    print(f"El DIŞARDA: {hareket_y}")
                ###                         ###
